...
server.on('request', async (request, response) => {
    ...
    */
    ...
    if (url_parts.pathname == '/submit') { //Processing the form content, if the relative URL is '/ submit'
        ...
        response.write(`<h1>Path: ${path}</h1>`); //Place given data (here: 'Hello' text) in the body of the answer
        if (typeof result === 'string') {
            // it is a file
            response.write(`<h3 style="color: blue">${result}</h3>`);
        } else if (result === undefined) {
            response.write(`<h3 style="color: green">is a directory!</h3>`)
        } else {
            response.write(
                `Such path does not exist: <h3 style="color: red">${result.message}</h3>`
            );
        }
